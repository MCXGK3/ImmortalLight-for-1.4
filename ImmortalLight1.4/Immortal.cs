...
{
    internal class Immortal : MonoBehaviour,Modding.ILogger
    {
        PlayMakerFSM _con;
        PlayMakerFSM _cho;
        PlayMakerFSM _com;
        ...
        private void Awake()
        {
            ...
            _cam = GameObject.Find("CameraParent").LocateMyFSM("CameraShake");
            Log(_cam);
        ...
        {
            AudioSource aus;
            aus = beam.AddComponent<AudioSource>();
            beam.SetActive(true);
            PlayMakerFSM beamFSM = beam.LocateMyFSM("Control");
            aus.PlayOneShot(BeamAnticClip, 0.3f * GameManager.instance.gameSettings.soundVolume / 10f);
            beamFSM.SendEvent("ANTIC");
            yield return new WaitForSeconds(antic);
            aus.PlayOneShot(BeamFireClip, 0.3f * GameManager.instance.gameSettings.soundVolume / 10f);
        ...
        private IEnumerator beginbeams(GameObject orb)
        {
            ...
            if (_cho.FsmVariables.FindFsmInt("Arena").Value != 1)
            {
        ...
        private static void FindALLChild(GameObject go, List<GameObject> list)
        {
        ...
        private void Start()
        {
            ...
            Log("Scene");
            Log("MODIFY");
            _con.InsertMethod("Tendrils1", () => { Log("PLAT"); ModifyPlatsPhase(); }, 0);
            ...
            for (int i = 0; i < 73; i++)
            {
                ...
                useorbs[i].LocateMyFSM("Orb Control").RemoveAction("Recycle", 0); 
            ...
            {
                orb.LocateMyFSM("Orb Control").InsertMethod("Stop Particles", () =>
                {
                    orb.SetActive(false);
                }, 3);
                orb.SetActive(false);
        ...
        private void ModifyCamera()
        {
            ...
            if(_cam!= null)
            { 
                foreach(var v in _cam.FsmVariables.Vector3Variables)
                {
                    vector3s.Add(v.Value);
                    v.Value = new Vector3(0, 0, 0);
                }
        ...
        { 
            ...
            foreach (var go in strings)
            {
                var ga=USceneUtil.Find(scene, go);
                if (ga != null) ga.SetActive(false);
            }
        }
        ...
        private void ModifyRage()
        {
            ...
            _con.InsertMethod("Climb Plats1", () =>
            {
                ...
                _com.FsmVariables.FindFsmGameObject("Eye Beam Glow").Value.SetActive(false);
            }, 0);
        ...
        {
            //改变位置
            _com.GetAction<DistanceBetweenPoints>("Orb Pos", 5).point2 = _com.FsmVariables.FindFsmVector3("Hero Pos");
            //加速
            ...
            _com.InsertMethod("Spawn Fireball", () =>
            {
        ...
        private void ModifySpike()
        {
            ...
            foreach (GameObject spike in spikes)
            {
                ...
                spike.LocateMyFSM("Hero Saver").InsertMethod("Send", () =>
                {
        ...
        {
            Log("MODIFYPLATS");
            GameObject.Find("Abyss Pit").LocateMyFSM("Ascend").RemoveTransition("Idle", "ASCEND");
            //常态
            _com.RemoveTransition("Aim Back", "FINISHED");
            _com.GetAction<Wait>("AB Start", 1).time = 0.02f;

            _con.InsertMethod("Tele Cast?", () =>
            {
                Log("TELE");
            ...
            _com.InsertMethod("Eb Extra Wait", () => {
            ...
            _com.InsertMethod("Final Event", () =>
            {
                AudioSource aus;
                aus = a2beams[0].AddComponent<AudioSource>();
                aus.PlayOneShot(BeamAnticClip, 0.3f * GameManager.instance.gameSettings.soundVolume / 10f);
                foreach (var beam in a2beams)
                {
            ...
            _com.InsertMethod("Comb L 2", () =>
            {
                int r = UnityEngine.Random.Range(6, 9);
                _com.FsmVariables.FindFsmGameObject("Attack Obj").Value.LocateMyFSM("Control").GetAction<SetPosition>("Top 2", 0).y = 58f;
                _com.FsmVariables.FindFsmGameObject("Attack Obj").Value.LocateMyFSM("Control").GetAction<SetFloatValue>("Top 2", 2).floatValue = 12f;
                _com.FsmVariables.FindFsmGameObject("Attack Obj").Value.LocateMyFSM("Control").GetAction<iTweenMoveBy>("Tween", 0).vector = new Vector3(0, 50, 0);

                _com.FsmVariables.FindFsmGameObject("Attack Obj").Value.LocateMyFSM("Control").FsmVariables.FindFsmInt("Type").Value = r;// > 2 ? r + 3 : r;
            }, 3);

            _com.InsertMethod("Comb R 2", () =>
            {
                int r = UnityEngine.Random.Range(6, 9);
                _com.FsmVariables.FindFsmGameObject("Attack Obj").Value.LocateMyFSM("Control").GetAction<SetPosition>("Top 2", 0).y = 58f;
                _com.FsmVariables.FindFsmGameObject("Attack Obj").Value.LocateMyFSM("Control").GetAction<SetFloatValue>("Top 2", 2).floatValue = 12f;
        ...
        private void ModifyAcend()
        {
            ...
            _com.InsertMethod("Comb Top 2", () =>
            {
                ...
                nails.LocateMyFSM("Control").InsertMethod("Antic", () =>
                {
                    List<GameObject> nailist = new List<GameObject>();
                    FindALLChild(nails.LocateMyFSM("Control").FsmVariables.FindFsmGameObject("Nails").Value,nailist);
            ...
            _con.InsertMethod("Plat Setup", () => {
                ...
                if (clock != null)
                    clock.Stop();
                clock = secondHand.AddComponent<AudioSource>();
                ...
                if (ImmortalLight1_4.Instance.setting.zoom)
                {
                    StartCoroutine(Zoomout());
                }
                if (ImmortalLight1_4.Instance.setting.performance)
                {
            ...
            _con.GetAction<Wait>("Plat Setup", 4).time = 0.5f;
            Log("Plat SetUP");
            _con.GetAction<SendEventByName>("Ascend Cast", 1).sendEvent = "COMB TOP2";
            Log("Ascend Cast");


            _con.InsertMethod("Scream", () => {
                _com.FsmVariables.FindFsmGameObject("Attack Obj").Value.LocateMyFSM("Control").SetState("Reset");
                GameObject.Find("Abyss Pit").LocateMyFSM("Ascend").AddTransition("Idle", "ASCEND", "Ascend");
        ...
        private void OnDestroy()
        {
            ...
            if (_cam != null) { 
                ...
                {
                    _cam.FsmVariables.Vector3Variables[i].Value = vector3s[i];
                }
                }
        }
        ...
        public static AudioClip LoadAudioClip(string path)
        {
            ...
            /*string text = Assembly.GetExecutingAssembly().GetManifestResourceNames().FirstOrDefault((string x) => x.EndsWith(".wav"));
        ...
        private void ShowConvo(string msg)
        {
            ...
            {
                playMakerFSM.InsertMethod("Set Convo", () =>
                {
        ...
        private void Log(object obj)
        {
            ...
            if (obj == null)
            {
                ImmortalLight1_4.Instance.Log(null);
            }
            else ImmortalLight1_4.Instance.Log("[immortallight]:" + obj);
        }
        ...
        public void Log(string message)
        {
            ...
            else ImmortalLight1_4.Instance.Log("[immortallight]:" + message);
        }

        void Modding.ILogger.Log(object message)
        {
